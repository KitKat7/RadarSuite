...
static float32_t testOutputTest[TEST_LENGTH];
static int kn = 0;
...
void arm_cfft_f32_app(void)
{
  ...
  arm_copy_f32(testInput, testOutputTest, 2048);
  //ifft test  
  for(i=0; i<1024; i++)
  {
    testOutput[i*2] = testInput[i*2];
...
//    Serial.printf("%f\t", testOutput[i*2+1]);
  ...
  for(i=0; i<1024; i++)
  {
    testOutput[i*2+1] = testInput[i*2+1];
  Serial.printf("%f\t", testOutput[i*2+1]);
  }
  Serial.printf("\n****Output Above****\n");
  ...
  for(i=0; i<1024; i++)
  {
  Serial.printf("%f\t", testOutputTest[i*2]);
//    Serial.printf("%f\t", testOutput[i*2+1]);
  }
  Serial.printf("\n****Output Above Test****\n");
  for(i=0; i<1024; i++)
  {
  Serial.printf("%f\t", testOutputTest[i*2+1]);
  }
  Serial.printf("\n****Output Above Test****\n");
