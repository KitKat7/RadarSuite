...
static q31_t testInput[TEST_LENGTH];
static q31_t testOutput[TEST_LENGTH];
static int kn = 0;
...
void arm_cfft_q31_app(void)
{
  ...
  {
    // testInput[i*2+1] = 0;
    // testInput[i*2] = arm_sin_q31(2*3.1415926f*50*i/1000);
    Serial.printf("%d\t", testInput[i*2]);
  }

   Serial.printf("\n****Input Above****\n");
   
  arm_cfft_radix4_q31(&S, testInput);
  arm_copy_q31(testInput, testOutput, 2048);
  //ifft test  
  for(i=0; i<1024; i++)
  {
    testOutput[i*2] = testOutput[i*2];
...
//    Serial.printf("%d\t", testOutput[i*2+1]);
  ...
  for(i=0; i<1024; i++)
  {    
    testOutput[i*2+1] = testOutput[i*2+1];
    Serial.printf("%d\t", testOutput[i*2+1]);
  }
  Serial.printf("\n****Output Above****\n");
    
  arm_cfft_radix4_init_q31(&S, fftSize, 1, doBitReverse);
  arm_cfft_radix4_q31(&S, testOutput);
  for(i=0; i<1024; i++)
  {
    testOutput[i*2] = testOutput[i*2];
    Serial.printf("%d\t", testOutput[i*2]);
